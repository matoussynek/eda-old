...
public class Ordenacion {
    ...
    */
    ...
    private static <T extends Comparable<T>> T[] mergeSort2(T[] v,
        ...
        if ( (f - i +1) == 2) {
            T[] tmp = (T[]) new Comparable[2];
            T val1 = v[i];
            T val2 = v[i + 1];
            
            tmp[0] = (val1.compareTo(val2)<0) ? val1 : val2;
            tmp[1] = (val1.compareTo(val2)>0) ? val1 : val2;
            
            return tmp;        
        }/*
        int mid = i + (f - i + 1)/2;
        T[] aux1 = (T[]) new Comparable[(f - i + 1)/2];
        T[] aux2 = (T[]) new Comparable[((f - i + 1)%2 == 0) ? ((f - i + 1)/2) : ((f - i + 1)/2 + 1) ];
        
        aux1 = mergeSort2(v,i,mid-1);
        aux2 = mergeSort2(v,mid, f);
        */
        int mid = (i+f)/2;
        T[] aux1 = mergeSort2(v,i,mid);
        T[] aux2 = mergeSort2(v,mid+1,f);
