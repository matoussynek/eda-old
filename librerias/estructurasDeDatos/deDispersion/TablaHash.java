...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    public ListaConPI<C> claves() {
        ...
        for(int i = 0; i < elArray.length;i++){
            ...
            l.inicio();
            while(!l.esFin()){
                temp.insertar(l.recuperar().clave);
                l.siguiente();
            }
    ...
    protected final void rehashing() {
        ListaConPI<EntradaHash<C, V>>[] temp = new LEGListaConPI[siguientePrimo(elArray.length*2)];
        
        for (int i = 0; i < temp.length; i++) { 
            temp[i] = new LEGListaConPI<EntradaHash<C, V>>();
        }
        ...
        for (int i = 0; i < elArray.length; i++){
            ...
            l.inicio();
            while(!l.esFin()){
                EntradaHash<C,V> entry = l.recuperar();
                int index = entry.clave.hashCode() % temp.length;
                
                temp[index].insertar(entry);
    ...
    public final double desviacionTipica() {
        double avg = factorCarga();
        double acc = 0;
        for (int i = 0; i < elArray.length; i++){
            int current = elArray[i].talla();
            acc += (current-avg)*(current-avg);
        }
    ...
    public String histograma() {
        ...
        for (int i = 0; i < elArray.length; i++){
            int current = elArray[i].talla();
            histogram[(current>9) ? 9 : current]+=1;
        }
        StringBuilder sb = new StringBuilder();
        for(int i=0; i<histogram.length; i++) {
            sb.append(i); sb.append("\t"); sb.append(histogram[i]); sb.append("\n");
        }
