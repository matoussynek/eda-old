...
public class Indexador {
        ...
        try {            
            ...
            while (libro.hasNext()) {
                ...
                for (int i = 0; i < words.length; i++) {
                    ...
                    if (esTermino(clave)) {
                        t1 = System.nanoTime();
                        //System.out.println(titulo + ", " + clave + ", " +numLin);
                        // COMPLETAR:
                        // añadir el nuevo par titulo, numLin (ind) al Map
                        // la clave puede haber aparecido antes o no.
                        ListaConPI<Indice> l = indices.recuperar(clave);
                        if (l == null) {
                            l = new LEGListaConPI<>();
                            l.inicio();
                            l.insertar(ind);
                            indices.insertar(clave, l);
                          }
                        else{
                            l.insertar(ind);
    ...
    public ListaConPI<String> indiceDe(String pal) {
        ...
        // COMPLETAR
        ListaConPI<Indice> temp = indices.recuperar(pal);
        temp.inicio();
        while(!temp.esFin()){
            res.insertar(temp.recuperar().toString());
            temp.siguiente();
        }
